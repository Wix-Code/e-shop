...
export const getCompareProducts = async (req, res) => { 
  ...
  try {
    ...
    const compare = user.compare

    return res.status(200).json({ success: true, message: "Compare list fetched", compare: compare });
  } catch (error) {
    console.log(error);
    res.status(500).json({ success: false, message: "Failed to fetch compare list" });
  }
}